...
class CoupleExtractor(CE):
	...
	CC_FILTER_FN = lambda x: x.pos_=='PUNCT' or x.dep_ in CE.CC_IDENTIFIER # punctuation and conjunctions
	...
	def is_passive(span): # return true if the sentence is passive - at the moment a sentence is assumed to be passive if it has an auxpass verb
		for token in span:
			if CE.get_token_dependency(token) in ["auxpass", 'oc']:
				return True
		return False
	...
	@staticmethod
	def get_token_extended_span(token): 
		if re.match(CE.AMOD_REGEXP, CE.get_token_dependency(token)) and not CoupleExtractor.is_verbal([token.head]):
			token = token.head
		return list(CE.get_composite_concept(token, None, lambda x: x)) # get the whole subtree
	...
	def get_couple_uid(couple):
		return (CE.get_word_dict_uid(couple['concept']), CE.get_word_dict_uid(couple['predicate']), couple['dependency'])
	...
	def trim_noise(token_list):
		def trim_fn(x):
			...
			return dep in CE.CC_IDENTIFIER+CE.PREP_IDENTIFIER+CE.PUNCT_IDENTIFIER
	...
	# 	core_path_to_inter = core_path_to_inter.difference(subj_obj_set)
	# 	if avoid_jumps:
	# 		jump_size = len(core_path_to_inter.intersection(core_set))
	# 		is_jumping = (jump_size > 1) if re.match(CE.AMOD_REGEXP, CE.get_token_dependency(core)) else (jump_size > 0) # greater than 1 instead of 0 allows for considering as concepts the cores which have a dependency in CE.AMOD_IDENTIFIER
	# 		if is_jumping:
	# 			return None
	# 	other_core_path_to_inter,other_core_junction = CE.find_path_to_closest_in_set(other_core,inter)
	# 	del inter
	...
	# 	other_core_path_to_inter = other_core_path_to_inter.difference(subj_obj_set)
	# 	if avoid_jumps:
	# 		jump_size = len(other_core_path_to_inter.intersection(core_set))
	# 		is_jumping = (jump_size > 1) if re.match(CE.AMOD_REGEXP, CE.get_token_dependency(other_core)) else (jump_size > 0) # greater than 1 instead of 0 allows for considering as concepts the cores which have a dependency in CE.AMOD_IDENTIFIER
	# 		if is_jumping:
	# 			return None
	# 	# Get predicate set
	# 	predicate_core_set = core_path_to_inter.union(other_core_path_to_inter)
	...
	def templatize_predicate_span(predicate_span, subj, obj):
		get_word_dict = lambda span: CoupleExtractor.get_word_dict_from_span(span)#, hidden_dep_list=CoupleExtractor.HIDDEN_PREDICATE_COMPONENT)
		...
		templatized_text = []
		if left_pivot > 0:
			left_pdict = get_word_dict(triple_span[:left_pivot])
		...
		if right_pivot > left_pivot+1:
			middle_pdict = get_word_dict(triple_span[left_pivot+1:right_pivot])
		...
		if right_pivot < len(triple_span)-1:
			right_pdict = get_word_dict(triple_span[right_pivot+1:])
	...
	def get_core_triplets(core_concept, other_core_concept, core_set, avoid_jumps=False): # can be one per core in core_set
		...
		verb_dict = {}
		if verb_span:
			verb_dict = get_word_dict(verb_span)
		...
		# get predicate_dict
		predicate_dict = get_word_dict(predicate_span)
		...
		predicate_dict['predicate_core'] = get_word_dict(predicate_core_span) # get predicate_core_dict
		...
		triplet_list = []
		subj_span = CoupleExtractor.get_token_extended_span(subj_core)
		obj_span = CoupleExtractor.get_token_extended_span(obj_core)
		# Considering the whole subj+obj subtrees reduces the number of source_text making question-answering more efficient without any loss of information
		predicate_text = get_word_dict(sorted(unique_everseen(predicate_span + subj_span + obj_span, key=lambda x: x.idx), key=lambda x: x.idx))['text']
		if verb_span:
			verb_text = get_word_dict(sorted(unique_everseen(verb_span + subj_span + obj_span, key=lambda x: x.idx), key=lambda x: x.idx))['text']
		for subj_dict in subj_dict_iter:
			for obj_dict in obj_dict_iter:
				...
				this_predicate_dict['source_text'] = predicate_text
				...
				if verb_span:
					this_verb_dict['source_text'] = verb_text
	...
	def get_triplet(core_concept, other_core_concept, core_set, avoid_jumps=False): # can be one per core in core_set
		...
		predicate_core_set = CoupleExtractor.get_predicate_core_between_concepts(core, other_core, core_set, avoid_jumps=avoid_jumps)
		if not predicate_core_set:
			ModelManager.logger.debug(f'empty predicate_core_set for <{core}> and <{other_core}>')
			return None
		# Enrich predicate set with details, adding hidden related concepts (n-ary relations)
		predicate_set = CoupleExtractor.expand_predicate_core(predicate_core_set, subj_obj_set=set((core,other_core)))
		# Add missing conjunctions
		...
		concept_dict_list = sorted(concept_dict_list, key=lambda x: CE.get_word_dict_size(x['concept']), reverse=True)
		other_concept_dict_list = sorted(other_concept_dict_list, key=lambda x: CE.get_word_dict_size(x['concept']), reverse=True)
		predicate_dict_span_set = set(predicate_dict['span'])
		concept_dict_iter = filter(lambda x: not predicate_dict_span_set.intersection(x['concept']['span']), concept_dict_list)
		other_concept_dict_iter = filter(lambda x: not predicate_dict_span_set.intersection(x['concept']['span']), other_concept_dict_list)
		...
		if not subj_dict or not obj_dict:
			ModelManager.logger.debug(f'no subj or obj found for <{core}> and <{other_core}>')
			return None

		subj_span = CoupleExtractor.get_token_extended_span(subj_core)
		obj_span = CoupleExtractor.get_token_extended_span(obj_core)
		# Considering the whole subj+obj subtrees reduces the number of source_text making question-answering more efficient without any loss of information
		predicate_dict['source_text'] = get_word_dict(sorted(unique_everseen(predicate_span + subj_span + obj_span, key=lambda x: x.idx), key=lambda x: x.idx))['text']
		predicate_dict['predicate_core'] = get_word_dict(predicate_core_span) # get predicate_core_dict
		...
		if verb_span:
			...
			verb_dict['lemma'] = ' '.join(verb_lemma)
			# Considering the whole subj+obj subtrees reduces the number of source_text making question-answering more efficient without any loss of information
			verb_dict['source_text'] = get_word_dict(sorted(unique_everseen(verb_span + subj_span + obj_span, key=lambda x: x.idx), key=lambda x: x.idx))['text']
		else:
			verb_dict = None
	...
	def get_triplet_list_by_concept_list(concept_list, avoid_jumps=False):
		# print(concept_list)
		ModelManager.logger.debug(f'Starting get_triplet_dict_list for {len(concept_list)} concepts')
		core_concept_dict = defaultdict(list)
		for concept in concept_list:
			for core_dict in concept['concept_core']:
				core_span = core_dict['span']
				if len(core_span) > 1:
					continue
				core = core_span[0]
		...
		for s,p,o in triplet_iter:
			...
			if 'source' in o:
				del o['source']
		ModelManager.logger.debug(f'Triplets count: {len(triplet_iter)}')
		return triplet_iter
	...
	def item_list_extraction_handler(self, get_item_fn, doc_parser: DocParser, parallel_extraction=True):
		...
		del content_iter
		del language_iter
		total = len(doc_content_annotation_language_iter)

		def _get_couple_list_by_doc(chunk, tqdm_fn=None):
			return flatten((
				get_item_fn(CE.get_concept_list_by_doc(*x))
				for x in (tqdm_fn(chunk, total=total, desc="Extracting couples") if tqdm_fn else chunk)
			), as_list=True)
		if not parallel_extraction:
			return _get_couple_list_by_doc(doc_content_annotation_language_iter, self.tqdm)
		...
		assert total == sum(map(len, chunks)), f"{total} != {sum(map(len, chunks))}"
		del doc_content_annotation_language_iter
		self.logger.info('CoupleExtractor::item_list_extraction_handler: starting Pool')
		with Pool(nodes=processes) as pool:
			partial_solutions = self.tqdm(pool.imap(_get_couple_list_by_doc, chunks), total=len(chunks), desc="Extracting couples")
