...
class AdjacencyRow:
	...
	def sort(self):
		self._in_.sort(key=str) # sort in-place
		self._out_.sort(key=str) # sort in-place
...
class AdjacencyList:
	...
	def get_predicate_chain(self, concept_set, direction_set, predicate_filter_fn=None, depth=None, already_explored_concepts_set=None):
		if depth is not None and depth <= 0:
			return concept_set
		if already_explored_concepts_set is None:
			already_explored_concepts_set = set()
		
		joint_set = set()
		new_concepts = concept_set
		while new_concepts and (depth is None or depth > 0):
			next_round_concepts = set()
			for c in new_concepts:
				if c in already_explored_concepts_set:
					continue
				already_explored_concepts_set.add(c)
				for direction in direction_set:
					adjacency_list = self.adjacency_list.get(c, None)
					if adjacency_list:
						for relation, target_concept in adjacency_list.get_generator(direction):
							if target_concept not in already_explored_concepts_set and (predicate_filter_fn is None or predicate_filter_fn(relation)):
								next_round_concepts.add(target_concept)
								joint_set.add(target_concept)
			new_concepts = next_round_concepts
			if depth is not None:
				depth -= 1

		return concept_set.union(joint_set)
	...
	def get_predicate_dict(self, predicate, manipulation_fn=lambda x: x): # Build labels dict
		...
		for s in self.get_nodes():
			for _,o in filter(lambda x: x[0]==predicate, self.get_outcoming_edges_matrix(s)):
				predicate_dict[s].append(manipulation_fn(o))
		# for k,v in predicate_dict.items():
		# 	predicate_dict[k] = tuple(sorted(v))
		for v in predicate_dict.values():
